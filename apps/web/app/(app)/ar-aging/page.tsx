 ...
 * AR aging — doc 04.
 *
 * The as-of date is explicit and sent as entered: an aging report is a
 * statement about a date, and one that silently used the clock could not be
 * reproduced tomorrow. The bucket columns carry the day ranges the server
 * actually used, read from the response rather than hardcoded, because the
 * ladder is the tenant's to choose.
 */
export default function ArAgingPage() {
  const entities = useResource(() =>
    api<{ data: { id: string; code: string; legal_name: string }[] }>('/legal-entities'),
  );
  // Contacts are a filter convenience; a report.view holder without customer.view
  // simply does not get the dropdown, and the report still runs unfiltered.
  const customers = useResource(() =>
    api<{ data: { id: string; legal_name: string }[] }>('/customers'),
  );
  ...
  const report = useResource(
    () =>
      applied
        ? api<AgingReport>('/reports/ar-aging', {
            query: {
  ...
  return (
    <>
      <PageHeader
        title="AR aging"
        description="Open customer balances bucketed by days overdue at an explicit date. The total reconciles to the AR control account — that reconciliation is Phase 3's first exit criterion."
      />
      ...
      <form onSubmit={run} className="card">
        ...
        {customers.data ? (
          <Field
            ...
            hint="Leave empty for all customers."
          />
        ) : null}
      ...
      {data ? (
        <>
          <DataTable<AgingRow>
            caption={`AR aging as of ${data.as_of}`}
            rows={data.data}
            rowKey={(row) => `${row.contact_id}-${row.currency}`}
            empty="Nothing outstanding at this date."
            columns={[
              { key: 'contact', header: 'Customer', render: (row) => row.contact_name },
